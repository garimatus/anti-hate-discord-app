...
export default async function() {
	...
	try {
		const queriesFiles = fs.readdirSync(path).filter(file => file.endsWith('.cql'));
        
        for (const file of queriesFiles) {
            // Set a new item in the Collection with the key as the command name and the value as the exported module
            const query = fs.readFileSync((await import("node:path")).join(path, file)).toString();
            
            const queryName = file.split(".")[0].toLowerCase().split("-")
                .reduce((prev, curr) =>
